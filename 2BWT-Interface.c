 ...
 * }
 */
//根据SA还原出seq在原始ref中的位置
//occ_pos is the position in packed reference(remove N in it)
void BWTRetrievePositionFromSAIndex(Idx2BWT * idx2BWT, unsigned int saIndex,
          unsigned int * sequenceId, unsigned int *ori_pos, unsigned int *occ_pos_res)
{
    unsigned int start, end, l, m, h, blockid;
 
    BWT * bwt = idx2BWT->bwt;
    HSP * hsp = idx2BWT->hsp;
           
    int nblock = hsp->numOfBlock;
    unsigned int occ_pos = BWTSaValue(bwt, saIndex);
    ChrBlock *blocklist = hsp->blockList;
    start = end = 0;
    l = 0; h = nblock;
    *occ_pos_res = occ_pos;
    ...
    while(l <= h) {
        ...
        /*  haven't consider strain */
        else if(start <= occ_pos && end >= occ_pos) {
            *sequenceId = blocklist[m].chrID;
            *ori_pos = occ_pos - start + (blocklist+m)->ori + 1;
            break;
        }
        else
            break;
...
// exact extend
// start: that position string we should extend
// leav_len: leaving length to extend
// type: 1 - backward, 0 - forward
int bwt_extend_exact(const Idx2BWT *bi_bwt, ubyte_t *seq, int start, int *leav_len, int type,
        bwtint_t *sa_begin, bwtint_t *sa_end, bwtint_t *rev_sa_begin, bwtint_t *rev_sa_end)
{
    ...
    if(type == 1){
        start -= (*leav_len);
        while( *leav_len != 0){
            real_pos = start +(*leav_len);
            if(seq[real_pos] > 3)
                break;
            BWTSARangeBackward_Bidirection(bi_bwt, seq[real_pos], &k, &l, &rev_k, &rev_l);
            if(k > l)
                break;
            else{
                *sa_begin = k;
        ...
        start = start + (*leav_len);
        while(*leav_len != 0)
        {
            real_pos = start - *leav_len;
            if(seq[real_pos] > 3)
                break; 
            BWTSARangeForward_Bidirection(bi_bwt, seq[real_pos], &k, &l, &rev_k, &rev_l);
            if(k > l)
                break;
            else{
                *sa_begin = k;
