        ...
        int type)
{
    ...
    // use reverse reference to test if a substring of read is also a substring of reference
    // but the direction is foreward
    if(type == 1) {
        for(i = 0 ; i < len; ++i){
            ubyte_t c = str[i];
            if(c < 4)
...
// TODO for debug, get seq real position from seq name
        ...
        const gap_opt_t *opt, bwt_array_t *arr)
{
    ...
    // init bwt_width_t 
    aux->width_back = (bwt_width_t *)calloc(max_len+1, sizeof(bwt_width_t));
    aux->width_fore = (bwt_width_t *)calloc(max_len+1, sizeof(bwt_width_t));
    aux->width_seed = (bwt_width_t *)calloc(max_len+1, sizeof(bwt_width_t));

    // init reverse complimetn seq
    aux->rc_seq = (ubyte_t *)calloc(max_len, sizeof(ubyte_t));
 ...
 */
    ...
    int _i, j, _j;
    ubyte_t *seq_cmp_a = (ubyte_t *)calloc(15, sizeof(ubyte_t));
    ubyte_t *seq_cmp_t = (ubyte_t *)calloc(15, sizeof(ubyte_t));
    memset(seq_cmp_t, 3, sizeof(ubyte_t) * 15);
 ...
 */
        ...
        p->n_aln = 0;
        p->aln = 0;
        if(memcmp(p->seq, seq_cmp_a, 15) == 0 || memcmp(p->seq, seq_cmp_t, 15)==0)
            continue;
        aux->seq = p->seq;
        aux->len = p->len;
...
void bwa_aln_core(const char *prefix, const char *fn_fa, const gap_opt_t *opt)
{
	...
	while ((seqs = bwa_read_seq(ks, 0x186A0, &n_seqs, opt->mode, opt->trim_qual)) != 0) {
		tot_seqs += n_seqs;
		t = clock();
 ...
 */
		...
		t = clock();
		fprintf(stderr, "[bwa_aln_core] write result(sam format) to the disk... \n");
        generate_sam_se_core(bi_bwt, n_seqs, seqs, opt, 3);
/* 		for (i = 0; i < n_seqs; ++i) {
 * 			bwa_seq_t *p = seqs + i;
