...
unsigned int HSPParseFASTAToPacked(const char* FASTAFileName, const char* annotationFileName, const char* packedDNAFileName, 
         const char* ambiguityFileName, const unsigned int FASTARandomSeed, const int maskLowerCase) {

	FILE *FASTAFile, *annotationFile, *packedDNAFile;
 ...
 */
	...
	chrAnnotation = (Annotation *)malloc(sizeof(Annotation)*chrAnnAllocated);
	...
	while(!feof(FASTAFile)){
		...
		if (chrNum == chrAnnAllocated){
			chrAnnAllocated <<= 1;
			chrAnnotation = (Annotation *)realloc(chrAnnotation, sizeof(Annotation)*chrAnnAllocated);
//			printf("%d\n", chrNum);
		}
